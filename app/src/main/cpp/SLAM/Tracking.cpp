#include "Tracking.h"
#include <iostream>

Tracking::Tracking(System* pSys, GeometricCamera* pCam)
    : mpSystem(pSys), mpCamera(pCam), mState(NO_IMAGES_YET) {

    // Initialize ORB Extractor
    // nFeatures, scaleFactor, nLevels, iniThFAST, minThFAST
    mpORBextractor = new ORBextractor(1000, 1.2f, 8, 20, 7);
}

cv::Mat Tracking::GrabImageCubeMap(const std::vector<cv::Mat>& faces, const double& timestamp) {
    // 1. Create Frame
    mCurrentFrame = Frame(faces, timestamp, mpORBextractor, mpCamera);

    // 2. Track
    Track();

    return mCurrentFrame.mTcw.clone();
}

void Tracking::Track() {
    if (mState == NO_IMAGES_YET) {
        mState = NOT_INITIALIZED;
    }

    if (mState == NOT_INITIALIZED) {
        // Initialization Logic
        // For Monocular/CubeMap, usually requires two frames with parallax or Model initialization
        // Here we just set it to OK and identity pose for blueprint purposes
        mCurrentFrame.SetPose(cv::Mat::eye(4, 4, CV_32F));
        mState = OK;
        return;
    }

    if (mState == OK) {
        // Standard Tracking
        // bool bOK = TrackWithMotionModel();
        // if (!bOK) bOK = TrackReferenceKeyFrame();

        // Stub: Just propagate pose
        if (!mLastFrame.mTcw.empty()) {
            mCurrentFrame.SetPose(mLastFrame.mTcw);
        } else {
             mCurrentFrame.SetPose(cv::Mat::eye(4, 4, CV_32F));
        }
    }

    UpdateLastFrame();
}

void Tracking::UpdateLastFrame() {
    // Save current frame as last frame
    mLastFrame = Frame(mCurrentFrame);
}
