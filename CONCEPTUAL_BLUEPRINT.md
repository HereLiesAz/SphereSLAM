...
* [ ] **Install NDK Side-by-Side:** via SDK Manager, install **NDK version r26b** (LTS). Avoid r27+ for now due to breaking changes in legacy C++ standard headers used by ORB-SLAM.
* [ ] **CMake Configuration:** Install **CMake 3.22.1**.
* [ ] **Project Structure:** Create a new project with "Native C++" template.
...
* [ ] **OpenCV 4.10 Android SDK:** Download pre-built Android SDK. Extract to `libs/opencv`.
* [ ] **Eigen 3.4.0:** Header-only. Copy to `app/src/main/cpp/Thirdparty/eigen`.
...
* [ ] **Manifest:** Request `CAMERA`, `HIGH_SAMPLING_RATE_SENSORS`.
* [ ] **Surface Setup:** Create two surfaces:
...
* [ ] **Sensor Sync:** Register `SensorEventListener` for **GYROSCOPE** and **ACCELEROMETER**.
* [ ] Timestamp Synchronization: Convert `sensorEvent.timestamp` (nanoseconds boot time) to match `cameraCaptureResult.sensorTimestamp`.
* [ ] **Critical:** Store IMU measurement in a lock-free ring buffer (e.g., `boost::circular_buffer` equivalent) to pass to the C++ SLAM thread.
...
* [ ] **Shader (comp.spv):** Write a GLSL compute shader.
...
* [ ] **Vulkan Pipeline:**
* [ ] Allocate `AHardwareBuffer` for zero-copy access.
* [ ] Bind the Input Bitmap as a texture.
* [ ] Dispatch Compute (Group size 16x16).
* [ ] Fence sync: Wait for GPU completion before signaling SLAM thread.
...
* [ ] **Remove Viewer:** Delete `Pangolin` dependency completely. We will render our own visualization.
* [ ] **Remove Atlas:** We only need the active map for this tool. Disable multi-map merging to save RAM.

### **3.2 Implementing the CubeMap Camera Model**

ORB-SLAM3 has `Pinhole` and `KannalaBrandt`. You must add `CubeMap`.

* [ ] **Class `GeometricCamera.h`:** Add `CubeMapCamera` class.
...
* [ ] **Unprojection Function (`Unproject`):**
...
* [ ] **FastCV Integration:** Replace `ORBextractor.cc` Gaussian Blur and FAST corner detection calls with Qualcomm `FastCV` equivalents (`fcvPyramidCreate`, `fcvCornerFast10`).
* [ ] **NEON Optimization:** If not using FastCV, ensure `ORBextractor.cc` is compiled with `-mfpu=neon`.
...
* [ ] **Download:** Metric3D or Depth-Any-Camera (ViT-Small variant).
* [ ] **Export to ONNX:**
```python
...
* [ ] **Quantize:** Use `onnx2tf` or `ai_edge_torch` to convert to TFLite Int8. *Crucial:* Do not use FP32 on mobile NPU; latency will be >1s. Int8 gets ~150ms.

### **4.2 Android Inference (LiteRT)**

* [ ] **TFLite Interpreter:** Initialize `InterpreterApi` with `NnApiDelegate` (Android 10+) or `GpuDelegate`.
* [ ] **Inference Loop:**
...
* [ ] **Scale Alignment:** Calculate the scale factor  by comparing SLAM feature depths with Model depths.
* 
...
* [ ] **Input:** The single photosphere + Depth Map.
* [ ] **Generation:** Instead of optimization (training), we procedurally generate Gaussians.
...
* [ ] **AHardwareBuffer:** Use strictly for passing image data between Camera -> Vulkan -> NPU. Never map to CPU memory (`ByteBuffer.get()`) if possible.
* [ ] **Double Buffering:** Implement ring buffers for the Gaussian data upload to GPU to prevent UI stalling.
...
* [ ] Set `minSdk 29` (Android 10) for AHardwareBuffer support.
...
This protocol provides a complete path from zero to a functioning SphereSLAM system. Execute phases sequentially to ensure component stability before integration.
